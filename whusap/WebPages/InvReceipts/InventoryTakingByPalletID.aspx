<%@ Page Title="" Language="C#" MasterPageFile="~/MDMasterPage.Master" AutoEventWireup="true"
    CodeBehind="InventoryTakingByPalletID.aspx.cs" Inherits="whusap.WebPages.InvReceipts.InventoryTakingByPalletID" %>
...
<style>
    ...
    <script>
        ...
        var VerificarQuantity = function () {
            $('#btnSave').prop("disabled", true);
            var Data = "{'CWAR':'" + $('#txWarehouse').val() + "','ITEM':'" + $('#lblItem').val() + "','LOCA':'" + $('#txLocation').val() + "','CLOT':'" + (ktlc == "1" ? $('#lblWorkOrder').html() : " ") + "'}";
            sendAjax("VerificarQuantity", Data, SuccesVerificarQuantity)
        }

        var Click_Save = function () {

            var Data = "{'PAID':'" + $('#txPalletID').val() + "','ITEM':'" + $('#lblItem').val() + "','CWAR':'" + $('#txWarehouse').val() + "','LOCA':'" + $('#txLocation').val() + "','UNIT':'" + $('#lblQuantity').html() + "','QTYS':'" + $('#txQuantity').val() + "'}";
        ...
        function sendAjax(WebMethod, Data, FuncitionSucces, asyncMode) {
            var options = {
                type: "POST",
                url: "InventoryTakingByPalletID.aspx/" + WebMethod,
                data: Data,
