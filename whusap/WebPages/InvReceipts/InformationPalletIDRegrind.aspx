<%@ Page Title="" Language="C#" MasterPageFile="~/MDMasterPage.Master" AutoEventWireup="true" CodeBehind="InformationPalletIDRegrind.aspx.cs" Inherits="whusap.WebPages.InvReceipts.InformationPalletIDRegrind" %>
...
<style>
    ...
    <script>
        ...
        var VerificarQuantity = function () {
            $('#btnSave').prop("disabled", true);
            var Data = "{'CWAR':'" + $('#txWarehouse').val() + "','ITEM':'" + $('#lblItem').html() + "','LOCA':'" + $('#txLocation').val() + "','CLOT':'" + (kltc == "1" ? $('#lblWorkOrder').html():" ")+ "'}";            
            //var Data = "{'CWAR':'" + $('#txWarehouse').val() + "','ITEM':'" + $('#txItem').val() + "','CLOT':'" + $('#txLot').val() + "'}";
        ...
        function sendAjax(WebMethod, Data, FuncitionSucces, asyncMode) {
            var options = {
                type: "POST",
                url: "InformationPalletIDRegrind.aspx/" + WebMethod,
                data: Data,
        ...
        txWarehouse.bind('paste keyup', function () {
            stoper();
            timer = setTimeout("VerificarWarehouse()", 500);
        });
