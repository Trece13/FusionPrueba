...
{
    public partial class whInvConfirm : System.Web.UI.Page
    {
        #region Propiedades
            ...
            public static DataTable _validaRecepcion = new DataTable();
        #endregion
        ...
        protected void btnConsultar_Click(object sender, EventArgs e)
            {
                ...
                {
                    _validaRecepcion = _idaltwhcol080.findRecordByOrnoSour(ref order, ref sour, ref strError);

                    if (_validaRecepcion.Rows.Count > 0)
                    {
                        makeTable();
                    ...
                    else
                    {
                        ...
                        lblError.Text = mensajes("deliverynotexists");
                        return;
                    }
                }
                else
        ...
        protected void makeTable()
        {
            ...
            table += String.Format("<hr /><table class='table table-bordered' style='font-size:13px; border:3px solid; border-style:outset; text-align:center;'>");

            table += String.Format("<tr style='font-weight:bold; background-color:lightgray;'><td>{0}</td><td colspan='3'>{1}</td></tr>"
                , _idioma == "INGLES" ? "Order:" : "Orden:"
                , txtOrder.Text.Trim().ToUpper());

            table += String.Format("<tr style='font-weight:bold; background-color:white;'><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td></tr>"
                , _idioma == "INGLES" ? "Item" : "Articulo"
                , _idioma == "INGLES" ? "Lot" : "Lote"
                , _idioma == "INGLES" ? "Suggest Qty" : "Cant sugerencia"
                , _idioma == "INGLES" ? "Confirm" : "Confirmar");


            for (int i = 0; i < _validaRecepcion.Rows.Count; i++)
            {
                table += String.Format("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td></tr>"
                    , _validaRecepcion.Rows[i]["ITEM"].ToString().Trim().ToUpper()
                    , _validaRecepcion.Rows[i]["CLOT"].ToString().Trim().ToUpper()
                    , _validaRecepcion.Rows[i]["QANA"].ToString().Trim().ToUpper()
                    , String.Format("<select id='{0}' name='{0}'><option value='true'>{1}</option><option value='false' selected>{2}</option></select>"
                    , "slConfirm-" + i
                    , _idioma == "INGLES" ? "Yes" : "Si"
                    , _idioma == "INGLES" ? "No" : "No"));
            }
        ...
        {
            lblMovement.Text = _textoLabels.readStatement(formName, _idioma, "lblMovement");
