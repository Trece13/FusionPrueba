...
namespace TECIT.OnlineBarcodes
{
  ...
  public class BarcodeHandler : IHttpHandler
  {
    ...
    private const string URLPARAM_Data = "&Data=";
    ...
    /// <summary>
    /// Set advanced parameters (bar code specific)
    /// </summary>
    private void SetAdvancedParams()
    {
      if (_segments > 0)
        _myBarcode.Rss.NumberOfSegmentsPerRow = _segments;
    }
    ...
    private void SetDPI()
    {
      // set DPI from GET parameter  
      _myBarcode.Dpi = GetValidatedDPI(_dpi);

    }

    /// <summary>
    /// Set Human Readable Text options
    /// </summary>
    private void SetBarodeHRT()
    {
      // set barcode font
      ...
      _myBarcode.IsTextVisible = _myBarcode.BarcodeType == BarcodeType.DAFT ? false : _printDataText;
    }

    /// <summary>
    /// Set bar code type and composite component
    /// </summary>
    ...
    /// <summary>
    /// Set barcode data and related options.
    /// </summary>
    ...
    private void SetBarcodeSize()
    {
        try
        {
            ...
            {
                // set default height  
                _myBarcode.BoundingRectangle = new Rectangle(0, 0, _myBarcode.BoundingRectangle.Width, _barcodeInfo.DefaultHeight);

                // temporarily switch off HRT
                // (otherwise HRT would be added to default height in CalculateOptimalBitmapSize)
                _myBarcode.IsTextVisible = false;
                
                // calculate optimal size for bitmaps
                Size optimalSize = _myBarcode.CalculateOptimalBitmapSize(null, _scaleX, _scaleY);
                
                // restore HRT
                _myBarcode.IsTextVisible = this._printDataText;

                // set height/width
                widthPx = optimalSize.Width;
                heightPx = optimalSize.Height;
            ...
            if (_myBarcode.IsTextVisible)
              heightPx = CalculateAutoFontHeight(widthPx, heightPx);

            // set width and height from GET parameters (if defined)
            switch (_unit)
            {
                case "mils":
            ...
            // set bounding rectangle to calculated size
        ...
        try
        {
            ...
            switch (key.ToLower())
            {
                ...
                case "data":
                    if (_rawUrl)
                    {
                      int i = _request.RawUrl.IndexOf(URLPARAM_Data, StringComparison.OrdinalIgnoreCase);
                      if (i >= 0)
                      {
                        _data = _request.RawUrl.Substring(i + URLPARAM_Data.Length);
                        _data = _data.Replace("%20", " "); // HttpUtility.UrlDecode(_data);
                      }
                    }
                    else
                      _data = value;
                ...
                case "scaley":
                    ...
                    break;
                case "segments":
                    _segments = int.Parse(value, numFormat);
                    break;
                default: break;
            }
        }
    ...
    /// <summary>
    /// Sets the width of the module and the size mode.
    /// </summary>
