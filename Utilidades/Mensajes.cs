...
namespace whusa.Utilidades
{
    public class Mensajes
    {
        ...
        public string readStatement(string Module, string idiom, ref string operationName)
        {
            var nombreMensaje = operationName;
            method = MethodBase.GetCurrentMethod();
            string resultado = string.Empty;
            strError = string.Empty;
            string recursoXML = resolveUrlAbsolute(ConfigurationManager.AppSettings["fileMessagesForms"].ToString());

            // Validar parametros de entrada
            if (string.IsNullOrEmpty(Module) || string.IsNullOrEmpty(operationName) || string.IsNullOrEmpty(idiom))
            ...
            try
            {
                ...
                module = result.Elements("operationName").FirstOrDefault(x => x.Attribute("name").Value == nombreMensaje);
